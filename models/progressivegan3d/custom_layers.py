...
def Upscale3d(x, factor=2):
    ...
    if factor == 1:
        return x
    s = x.size()
    x = x.view(-1, s[1], s[2], 1, s[3], 1, s[4], 1)
    x = x.expand(-1, s[1], s[2], factor, s[3], factor, s[4], factor)
    x = x.contiguous().view(-1, s[1], s[2] * factor, s[3] * factor, s[4]*factor)
    return x
...
class EqualizedConv3d(ConstrainedLayer):
    ...
    def __init__(self,
        ...
        ConstrainedLayer.__init__(self,
                                  nn.Conv3d(nChannelsPrevious, nChannels,
