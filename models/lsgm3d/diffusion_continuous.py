...
import torch
import gc
from .utils import trace_df_dx_hutchinson, sample_gaussian_like, get_mixed_prediction
from ..nvae3d.distributions import log_p_var_normal, log_p_standard_normal
from torchdiffeq import odeint
from timeit import default_timer as timer


def make_diffusion(sde_type, sigma2_0, sigma2_min, sigma2_max, time_eps, beta_end, beta_start):
    """ Simple diffusion factory function to return diffusion instances. Only use this to create continuous diffusions """
    if sde_type == 'geometric_sde':
        return DiffusionGeometric(sigma2_0, sde_type, sigma2_min, sigma2_max)
    elif sde_type == 'vpsde':
        return DiffusionVPSDE(sigma2_0, sde_type, time_eps, beta_end, beta_start)
    elif sde_type == 'sub_vpsde':
        return DiffusionSubVPSDE(sigma2_0, sde_type, time_eps, beta_end, beta_start)
    elif sde_type == 'vesde':
        return DiffusionVESDE(sigma2_0, sde_type, sigma2_min, sigma2_max)
    else:
        raise ValueError("Unrecognized sde type: {}".format(sde_type))
...
class DiffusionBase(ABC):
    ...
    """
    def __init__(self, sigma2_0, sde_type):
        super().__init__()
    ...
    def compute_ode_nll(self, dae, eps, ode_eps, ode_solver_tol, no_autograd, num_samples, report_std):
        ...
        for i in range(num_samples):
            ...
            if self.sde_type == 'vesde':
                logp_prior = torch.sum(log_p_var_normal(x_t0, var=self.sigma2_max), dim=[1, 2, 3, 4])
            else:
                logp_prior = torch.sum(log_p_standard_normal(x_t0), dim=[1, 2, 3, 4])
    ...
    def sample_model_ode(self, dae, num_samples, shape, ode_eps, ode_solver_tol, temp, noise=None):
    ...
    def _iw_quantities_vpsdelike(self, size, time_eps, iw_sample_mode):
        ...
        return t, var_t.view(-1, 1, 1, 1, 1), m_t.view(-1, 1, 1, 1, 1), obj_weight_t.view(-1, 1, 1, 1, 1), \
            obj_weight_t_ll.view(-1, 1, 1, 1, 1), g2_t.view(-1, 1, 1, 1, 1)
...
class DiffusionGeometric(DiffusionBase):
    ...
    """
    def __init__(self, sigma2_0, sde_type, sigma2_min, sigma2_max):
        super().__init__(sigma2_0, sde_type)
...
class DiffusionVPSDE(DiffusionBase):
    ...
    """
    def __init__(self, sigma2_0, sde_type, time_eps, beta_end, beta_start):
        super().__init__(sigma2_0, sde_type)
