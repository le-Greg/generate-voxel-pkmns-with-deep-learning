...
import torch
import numpy as np
from .utils import get_mixed_prediction
from ..nvae3d.distributions import PixelNormal
...
class DiffusionDiscretized(object):
    ...
    """
    def __init__(self, denoising_stddevs, diffusion_steps, var_fun):  # alpha_bars_fun
        self._diffusion_steps = diffusion_steps
        self._denoising_stddevs = denoising_stddevs
    ...
    def sample_q(self, x_init, noise, timestep):
        ...
        alpha_bars_sqrt = torch.sqrt(alpha_bars).view(-1, 1, 1, 1, 1).expand(size)
        one_minus_alpha_bars_sqrt = torch.sqrt(1.0 - alpha_bars).view(-1, 1, 1, 1, 1).expand(size)
    ...
    def _extract_loss_weights(self, timestep, size):
        ...
        weights_invsigma2_term = torch.gather(self._weights_invsigma2_term, 0, timestep-1).view(-1, 1, 1, 1, 1).expand(size)
        weights_const_term = torch.gather(self._weights_const_term, 0, timestep-1).view(-1, 1, 1, 1, 1).expand(size)
        weights_l2_term = torch.gather(self._weights_noisepred_l2_term, 0, timestep-1).view(-1, 1, 1, 1, 1).expand(size)
    ...
    def reconstruction_loss(self, decoder, x, timestep=None, weighted_loss=False):
        ...
        return torch.sum(recloss, dim=[1, 2, 3, 4])

    def run_denoising_diffusion(self, model, num_samples, shape, temp=1.0, is_image=True, prior_var=1.0):
        """
        ...
        with torch.no_grad():
            for t in reversed(range(0, self._diffusion_steps)):
                ...
                logits = get_mixed_prediction(model.mixed_prediction, pred_logits, model.mixing_logit, mixing_component)

                output_dist = PixelNormal(logits, fixed_log_scales)
    ...
    def compute_nelbo(self, model, x_init, is_image=True, prior_var=1.0):
    ...
    def calculate_base_kl(self, x_init, prior_var):
        ...
        kl = 0.5 * (self._alpha_bars[-1] * torch.square(x_init)
            ...
            - 0.5 
        return torch.sum(kl, dim=[1, 2, 3, 4])
