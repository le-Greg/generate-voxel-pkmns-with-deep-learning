...
from .utils import init_temb_fun
...
class NCSNpp(nn.Module):
    ...
    def __init__(self, num_input_channels, num_scales_dae, num_channels_dae, dae_channel_multiplier, num_cell_per_scale_dae,
                 dropout, im_size, num_preprocess_blocks, num_latent_scales, mixed_prediction, mixing_logit_init, fir,
                 progressive, progressive_input, progressive_combine, embedding_dim, embedding_type, embedding_scale):
        super().__init__()
        self.act = act = nn.SiLU()
        self.num_scales = num_scales_dae
        self.num_input_channels = num_input_channels

        self.nf = nf = num_channels_dae
        ch_mult = dae_channel_multiplier[:self.num_scales]
        self.num_res_blocks = num_res_blocks = num_cell_per_scale_dae
        ...
        self.input_size = im_size // 2 ** (num_preprocess_blocks + num_latent_scales - 1)
        ...
        if self.mixed_prediction:
            init = mixing_logit_init * torch.ones(size=[1, num_input_channels, 1, 1, 1])
        ...
        self.progressive_input = progressive_input = progressive_input
        combine_method = progressive_combine
        combiner = functools.partial(Combine, method=combine_method)

        self.embedding_dim = embedding_dim
        self.embedding_dim_mult = 4
        self.temb_fun = init_temb_fun(embedding_type, embedding_scale, embedding_dim)
