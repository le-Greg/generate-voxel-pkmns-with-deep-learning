...
class PixelShuffle3d(nn.Module):
    """
    This is a 3d implementation of Pytorch's PixelShuffle
    Found here : https://github.com/gap370/pixelshuffle3d
    ...
    def forward(self, x):
        ...
        input_view = x.contiguous().view(batch_size, nOut, self.upscale_factor, self.upscale_factor,
                                         self.upscale_factor, in_depth, in_height, in_width)
...
class ConvMeanPool(nn.Module):
    ...
    def forward(self, inputs):
        ...
        output = sum([output[:, :, ::2, ::2, ::2], output[:, :, 1::2, ::2, ::2],
                      ...
                      output[:, :, ::2, 1::2, 1::2], output[:, :, 1::2, 1::2, 1::2]
                      ]) / 8.
        return output
...
class UpsampleConv(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size=3, biases=True):
        super().__init__()
        self.conv = nn.Conv3d(input_dim, output_dim, kernel_size, stride=1, padding=kernel_size // 2, bias=biases)
        self.pixelshuffle = PixelShuffle3d(upscale_factor=2)
...
class AttnBlock(nn.Module):
    ...
    def forward(self, x):
        # TODO Didn't test the 3d implementation of this
        B, C, H, W, D = x.shape
        ...
        w = torch.reshape(w, (B, H, W, D, H * W * D))
        w = F.softmax(w, dim=-1)
        w = torch.reshape(w, (B, H, W, D, H, W, D))
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W, D = x.shape
        h = F.interpolate(x, (H * 2, W * 2, D * 2), mode='nearest')
        if self.with_conv:
            h = self.Conv_0(h)
        return h
...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        ...
        assert x.shape == (B, C, H // 2, W // 2, D // 2)
        return x
