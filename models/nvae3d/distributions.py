...
class PixelNormal(object):
    def __init__(self, param, fixed_log_scales=None):
        ...
        if fixed_log_scales is None:
            ...
            self.log_scales = fixed_log_scales.view(-1, 1, 1, 1, 1).expand(size)                     # B, 1 or 3, H, W
...
class DiscMixLogistic:
    def __init__(self, param, num_mix=10, num_bits=8):
        ...
        l = param[:, num_mix:, :, :].view(B, 3, 3 * num_mix, H, W, D)                    # B, 3, 3 * M, H, W
    ...
    def log_p(self, samples):
        ...
        samples = samples.unsqueeze(5)                                                  # B, 3, H , W
        samples = samples.expand(-1, -1, -1, -1, -1, self.num_mix).permute(0, 1, 5, 2, 3, 4)   # B, 3, M, H, W
