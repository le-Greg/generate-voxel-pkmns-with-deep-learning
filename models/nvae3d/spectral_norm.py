...
from .layers.neural_operations import Conv3D
from .layers.neural_ar_operations import ARConv3d
...
class SpectralNormCalculator:
    ...
    def add_conv_layers(self, model):
        for n, layer in model.named_modules():
            if self.custom_conv:
                if isinstance(layer, Conv3D) or isinstance(layer, ARConv3d):  # add our customized conv layers
                    self.all_conv_layers.append(layer)
            else:
                if isinstance(layer, nn.Conv3d):  # add pytorch conv layers
                    self.all_conv_layers.append(layer)

    def add_bn_layers(self, model):
        for n, layer in model.named_modules():
            if isinstance(layer, nn.BatchNorm3d) or isinstance(layer, nn.SyncBatchNorm) or\
