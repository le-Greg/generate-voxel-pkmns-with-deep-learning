...
def create_conv_mask(kernel_size, c_in, g_in, c_out, zero_diag, mirror):
    m = (kernel_size - 1) // 2
    mask = np.ones([c_out, c_in // g_in, kernel_size, kernel_size, kernel_size], dtype=np.float32)
    mask[:, :, m:, :, :] = 0
    mask[:, :, m, :m, :] = 1
    mask[:, :, m, m, :m] = 1
    mask[:, :, m, m, m] = channel_mask(c_in, g_in, c_out, zero_diag)
    if mirror:
        mask = np.copy(mask[:, :, ::-1, ::-1, ::-1])
    return mask
...
class ARConv3d(nn.Conv3d):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False,
        ...
        """
        super(ARConv3d, self).__init__(C_in, C_out, kernel_size, stride, padding, dilation, groups, bias)
        ...
        init = torch.log(norm(self.weight * init_mask, dim=[1, 2, 3, 4]).view(-1, 1, 1, 1, 1) + 1e-2)
    ...
    def normalize_weight(self):
        ...
        if self.masked:
            ...
            weight = weight * self.mask.to(weight.device)
...
class ELUConv(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, padding=0, dilation=1, masked=True, zero_diag=True,
        ...
        self.conv_0 = ARConv3d(C_in, C_out, kernel_size, stride=1, padding=padding, bias=True, dilation=dilation,
...
class ARInvertedResidual(nn.Module):
    def __init__(self, inz, inf, ex=6, dil=1, k=5, mirror=False):
        ...
        padding = dil * (k - 1) // 2
        layers = []
        layers.extend([ARConv3d(inz, hidden_dim, kernel_size=3, padding=1, masked=True, mirror=mirror, zero_diag=True),
                       nn.ELU(inplace=True)])
        layers.extend([ARConv3d(hidden_dim, hidden_dim, groups=hidden_dim, kernel_size=k, padding=padding, dilation=dil,
...
class MixLogCDFParam(nn.Module):
    ...
    def forward(self, ftr):
        out = self.conv(ftr)
        b, c, h, w, d = out.size()
        out = out.view(b, self.num_z, c // self.num_z, h, w, d)
        m = self.num_mix
