...
def get_skip_connection(Cin, Cout, stride):
    if stride == 1:
        ...
        return Conv3D(Cin, Cout, 1, stride=2, padding=0, bias=True)
    elif stride == -1:
        return nn.Sequential(UpSample(), Conv3D(Cin, Cout, kernel_size=1))
...
def normalize_weight_jit(log_weight_norm, weight):
    ...
    wn = torch.sqrt(torch.sum(weight * weight, dim=[1, 2, 3, 4]))   # norm(w)
    weight = n * weight / (wn.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) + 1e-5)
    return weight
...
class Conv3D(nn.Conv3d):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False, data_init=False,
        ...
        """
        super(Conv3D, self).__init__(C_in, C_out, kernel_size, stride, padding, dilation, groups, bias)

        self.log_weight_norm = None
        if weight_norm:
            init = norm(self.weight, dim=[1, 2, 3, 4]).view(-1, 1, 1, 1, 1)
    ...
    def forward(self, x):
        ...
        if self.data_init and not self.init_done:
            with torch.no_grad():
                weight = self.weight / (norm(self.weight, dim=[1, 2, 3, 4]).view(-1, 1, 1, 1, 1) + 1e-5)
                bias = None
                out = F.conv3d(x, weight, bias, self.stride, self.padding, self.dilation, self.groups)
                mn = torch.mean(out, dim=[0, 2, 3, 4])
                st = 5 * torch.std(out, dim=[0, 2, 3, 4])

                if self.bias is not None:
                    self.bias.data = - mn / (st + 1e-5)
                self.log_weight_norm.data = -torch.log((st.view(-1, 1, 1, 1, 1) + 1e-5))
                self.init_done = True

        self.weight_normalized = self.normalize_weight()

        bias = self.bias
        return F.conv3d(x, self.weight_normalized, bias, self.stride,
...
class GroupNormSwish(nn.Module):
    def __init__(self, C_in, eps=BN_EPS):
...
def get_batchnorm(C_in, eps=BN_EPS, momentum=0.05):
    if SYNC_BN:
        ...
        return nn.BatchNorm3d(C_in, eps, momentum)
...
class BNSwishConv(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride=1, padding=0, dilation=1):
        ...
        stride = abs(stride)
        self.bn_act = nn.Sequential(nn.BatchNorm3d(C_in, eps=BN_EPS, momentum=0.05), Swish())
        self.conv_0 = Conv3D(C_in, C_out, kernel_size, stride=stride, padding=padding, bias=True, dilation=dilation)
...
class BNSwishConvX2(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride=1, padding=0, dilation=1):
        ...
        self.upsample = stride == -1
        stride = abs(stride)
        self.bn_act_0 = nn.Sequential(nn.BatchNorm3d(C_in, eps=BN_EPS, momentum=0.05), Swish())
        self.conv_0 = Conv3D(C_in, C_out, kernel_size, stride=stride, padding=padding, bias=False, dilation=dilation,
                             weight_norm=False)
        self.bn_act_1 = nn.Sequential(nn.BatchNorm3d(C_out, eps=BN_EPS, momentum=0.05), Swish())
        self.conv_1 = Conv3D(C_out, C_out, kernel_size, stride=1, padding=padding, bias=True, dilation=dilation,
                             weight_norm=True)
...
class GNSwishConv(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride=1, padding=0, dilation=1, dropout_prop=0.):
        ...
        self.conv0 = Conv3D(C_in, C_out, kernel_size, stride=stride, padding=padding, bias=True, dilation=dilation, weight_norm=False)
        self.gn_act1 = GroupNormSwish(C_out, eps=BN_EPS)
        self.conv1 = Conv3D(C_out, C_out, kernel_size, stride=1, padding=padding, bias=True, dilation=dilation, weight_norm=False)
...
class EncCombinerCell(nn.Module):
    def __init__(self, Cin1, Cin2, Cout, cell_type):
        ...
        self.conv = Conv3D(Cin2, Cout, kernel_size=1, stride=1, padding=0, bias=True)
...
class DecCombinerCell(nn.Module):
    def __init__(self, Cin1, Cin2, Cout, cell_type):
        ...
        self.cell_type = cell_type
        self.conv = Conv3D(Cin1 + Cin2, Cout, kernel_size=1, stride=1, padding=0, bias=True)
...
class ConvBNSwish(nn.Module):
    def __init__(self, Cin, Cout, k=3, stride=1, groups=1, dilation=1):
        ...
        self.conv = nn.Sequential(
            Conv3D(Cin, Cout, k, stride, padding, groups=groups, bias=False, dilation=dilation, weight_norm=False),
...
class InvertedResidual(nn.Module):
    def __init__(self, Cin, Cout, stride, ex, dil, k, g):
        ...
        layers = [get_batchnorm(Cin, eps=BN_EPS, momentum=0.05),
                  ...
                  Conv3D(hidden_dim, Cout, 1, 1, 0, bias=False, weight_norm=False),
...
class InvertedResidualGN(nn.Module):
    def __init__(self, Cin, Cout, stride, ex, dil, k, g):
        ...
        layers2 = [ConvGNSwish(self.hidden_dim, self.hidden_dim, stride=self.stride, groups=groups, k=k, dilation=dil),
                   Conv3D(self.hidden_dim, Cout, 1, 1, 0, bias=False, weight_norm=False),
...
class Attention(nn.Module):
    def __init__(self, Cin):
        ...
        self.qkv = Conv3D(Cin, 3*Cin, kernel_size=1, stride=1, padding=0, weight_norm=True, bias=True)

    def forward(self, x):
        ftr = x
        hx, wx, dx = x.size(2), x.size(3), x.size(4)
        ...
        b, c, h, w, d = qkv.shape
        qkv = qkv.view(b, c, h * w * d)             # b, 3*c, hwd
...
class IdentityWithBackwardClipNorm(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        cutoff = ctx.cutoff
        hwd = grad_output.shape[2] * grad_output.shape[3] * grad_output.shape[4]
        norm_per_channel_normalized = torch.norm(grad_output, p=2, dim=[2, 3, 4], keepdim=True) / hwd
