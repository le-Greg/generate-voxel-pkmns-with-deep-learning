...
from .layers.neural_operations import EncCombinerCell, DecCombinerCell, Conv3D, InputSkipCombinerCell
from .layers.cells import Cell, PairedCellAR
from .distributions import Normal, DiscMixLogistic, DiscLogistic, Bernoulli
...
class NVAE(nn.Module):
    def __init__(self, use_se, arch_instance, img_size, num_channels_img, device, log_sig_q_scale, num_x_bits, decoder_dist,
                 num_latent_scales, num_groups_per_scale, num_latent_per_group, num_channels_enc,
                 num_preprocess_blocks, num_preprocess_cells, num_cell_per_cond_enc, num_channels_dec,
                 num_postprocess_blocks, num_postprocess_cells, num_cell_per_cond_dec, progressive_input_vae,
                 channel_mult, num_nf):
        super(NVAE, self).__init__()
        self.arch_instance = get_arch_cells(arch_instance, use_se)
        self.crop_output = False
        self.device = device
        self.log_sig_q_scale = torch.Tensor([log_sig_q_scale]).to(device)
        ...
        # progressive input
        self.progressive_input = progressive_input_vae
        ...
        self.channels_mult = channel_mult
        ...
        prior_ftr0_size = (int(c_scaling * self.num_channels_dec), self.input_size // spatial_scaling,
                           self.input_size // spatial_scaling, self.input_size // spatial_scaling)
    ...
    def init_stem(self):
        ...
        Cin = self.num_channels_img
        stem = Conv3D(Cin, Cout, kernel_size=3, padding=1, bias=True)
        return stem
    ...
    def init_normal_sampler(self, scale_ind):
        ...
        for s in range(self.num_latent_scales):
            for g in range(self.groups_per_scale[self.num_latent_scales - s - 1]):
                ...
                cell = nn.Sequential(
                    nn.ELU(),
                    Conv3D(num_c, 2 * self.num_latent_per_group, kernel_size=3, padding=1, bias=True)
                )
                ...
                cell = Conv3D(self.num_latent_per_group, self.num_latent_per_group, kernel_size=3, padding=1, bias=True)
    ...
    def init_image_conditional(self, scale_ind):
        ...
        return nn.Sequential(nn.ELU(),
                             Conv3D(C_in, C_out, 3, padding=1, bias=True))
            ...
            for cell in self.dec_tower:
                if cell.cell_type == 'combiner_dec':
                    if idx_dec < num_eps_z_given:
                        ...
                        b, _, h, w, d = s.shape
                        size = [b, self.num_latent_per_group, h, w, d]
